...
class Reward
{
    ...
    public function splitReward($rewardMoney,$rewardNum,$max,$min)
    {
        ...
        if($rewardArr[0] > $max) {
        	#差额
        	$diff = 0;
            $this->diff($diff, $rewardArr, $max, $min);
        }
        return $rewardArr;
    }

    #处理所有超过最大值的红包
    public function diff($diff, &$rewardArr, $max, $min)
    {
        rsort($rewardArr);
        ...
        if (round($diff/(count($rewardArr)-$k+1)) > 0) {
        	$i++;
            $this->diff($diff, $rewardArr, $max, $min, $i);
        }
    }

    #第一个红包小于0,从大红包上往下减
    public function add(&$rewardArr, $min, $i = 0){
    	echo "第".($i+1).'次进入<br/>';#$i是测试时用来记录递归次数的,基本一两次就修正好了
